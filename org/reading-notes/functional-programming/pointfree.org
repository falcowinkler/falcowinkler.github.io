...
* TODO Dependency Injection Made modular
Introduces the ~Environment~ generic parameter to the reducer for dependency injection.
Describes how to model _environment as tuples_.
The approach of modelling the environment as tuple is not really scaleable in a TCA project,
since this is giving up exporting noop/failing dependencies etcs.
...
- Introduces the basics of vanilla SwiftUI, not much new beyond basics of SwiftUI API's.
- Hints at the idea to model all navigation states of the app as a single data structure (a tree).
* SwiftUI Navigation: Tabs & Alerts, Part 2
Problematic about the vanilla way to model an optional alert is that it introduces _invalid states_.
For example, the boolean might be set to true (meaning an alert will be shown), without any actual data to show.
- Better use the ~alert(Binding<Item>, Item -> View)~ overload 😉.
- use IdentifiedArrayOf<T> for items where you later need access to indices (e.g. to delete items).
- ~@State~ bad because hard to test, use view models instead. 

Some of the nice overloads have been deprectated. Use these wrappers instead for sheets and alerts.
...
TCA introduces /structured concurrency tools/.
~try~ effect opens an async context in which the ~send~ parameter can be used to send actions
back into the system.

To fully make use of this with SwiftUI's new concurrency tools, make sure to add this to SwiftUI's import.
#+BEGIN_SRC swift
@preconcurrency import SwiftUI
#+END_SRC

** Test changes
1) TestSuite needs to be ~@MainActor~
2) Mark each test function as ~async~
3) store.send etc. needs to be ~await~'ed.

** Dependencies
- You don't need the effect type anymore for dependencies. Don't need to compile TCA for just a library
- Effects that return a single value can be async (throwing) functions
- Effects that return many effects can be ~AsyncStream~ or ~AsyncThrowingStream~
- Replace ~.failing~ effects with ~XCTUnimplemented~, use ~placeholder~ where a return value is needed.
- ~.finished~ can be used to define ~AsyncStreams~ that finish immediately.
- ~future~ is replaced with ~await withUnsafeContinuation~
** Reducer
- Use ~TaskResult~ instead of ~Result~
- Use ~.run~ to create async effects, ~catch~ trailing closure to handle errrors
- ~await withThrowingTaskGroup~ for parallel work
- ~withCancellation~ can be used to make only part of a task group cancellable.
